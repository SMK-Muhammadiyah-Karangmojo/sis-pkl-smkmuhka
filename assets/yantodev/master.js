...
async function addPendamping() {
    fetchingData('/api/v1/tutor/add', {
        ...
        if (response.responseData.responseCode === 200) {
            ...
            Swal.fire({
                icon: "error", title: responseData.responseCode, text: responseData.responseMsg
            })
        }
    })
}
...
async function editTutor(id) {
    ...
    Swal.fire({
        ...
        preConfirm: async () => {
            ...
            fetchingData('/api/v1/tutor/update-tutor/', data)
                .then(response => {
                    if (response.responseData.responseCode === 200) {
                        Swal.fire({
                            icon: 'success', title: "data updated successfully!!!",
                        });
                        setTimeout(function () {
                            window.location.reload(1);
                        }, 2000);
...
async function findTutorById(id) {
    return fetchingData('/api/v1/tutor/find-by-id', {id})
        .then(response => {
            if (response.responseData.responseCode === 200) {
...
async function findIdukaById(id) {
    return fetchingData('/api/v1/iduka/detail', {id})
        .then(response => {
            if (response.responseData.responseCode === 200) {
                return response.result
            }
...
async function findIdukaByTp(tp) {
    let iduka = [];
    await fetchingData('/api/v1/iduka/find-all-by-tp/' + tp)
        .then(response => {
            for (const element of response.result) {
                let id = element.id;
                let name = element.name;
...
async function findIdukaByMajor(major) {
    let iduka = [];
    await fetchingData('/api/v1/iduka/find-all-by-major/' + major)
        .then(response => {
            if (response.responseData.responseCode === 200) {
                for (const element of response.result) {
                    let id = element.id;
