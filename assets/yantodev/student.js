...
async function editStudent(id, masterDataId) {
    ...
    Swal.fire({
        ...
        preConfirm: async () => {
            fetchingData('/api/v1/student/update-master-data-student/', {
                id: masterDataId,
                nis: student.nis,
                tp: student.tpId,
                userPublicId: student.id,
...
async function getDetailStudent(id) {
    return fetchingData('/api/v1/student/find-by-id', {id})
        .then(response => {
            if (response.responseData.responseCode === 200) {
...
async function updateIdukaStudent(nis, id) {
    ...
    Swal.fire({
        title: `Apakah kamu yakin akan PKL di ${iduka}?`,
        icon: "warning",
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
            fetchingData('/api/v1/student/update-iduka', {
                nis, id
            }).then(response => {
                if (response.responseData.responseCode === 200) {
                    Swal.fire({
...
async function findMasterSubLaporan(id) {
    let result = [];
    await fetchingData('/api/v1/student/find-sub-report',
        {id}
    ).then(response => {
...
async function findSubLaporan(id) {
    let result = [];
    await fetchingData('/api/v1/student/find-sub-sub-report',
        {id}
    ).then(response => {
...
function deleteReport(id) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            fetchingData("/api/v1/student/delete-report", {id})
                .then(response => {
                    if (response.responseData.responseCode === 200) {
                        Swal.fire("Success", "Laporan kamu berhasil terhapus..", "success");
                    }
                })
            setTimeout(function () {
                window.location.reload(1);
